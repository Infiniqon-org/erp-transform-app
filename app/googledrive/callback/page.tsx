...
function GoogleDriveCallbackContent() {
  ...
  const handleCallback = async () => {
    try {
      ...
      if (success === 'true') {
        ...
        if (window.opener) {
          window.opener.postMessage(
            { type: 'googledrive-auth-success', state },
            window.location.origin
          )
          setTimeout(() => window.close(), 1500)
        } else {
          setTimeout(() => router.push('/files?storage=connected'), 2000)
        }
      } else {
      ...
      setStatus('error')
      setMessage((err as Error).message || 'Failed to connect to Google Drive')

      if (window.opener) {
        window.opener.postMessage(
          { type: 'googledrive-auth-error', error: (err as Error).message || 'Connection failed' },
          window.location.origin
        )
        setTimeout(() => window.close(), 5000)
      } else {
        setTimeout(() => router.push('/files?storage=error'), 3000)
      }
    }
  }
      ...
      <div className="bg-card border rounded-xl shadow-lg p-8 max-w-sm w-full">
        {status === 'processing' && (
          <div className="text-center">
            ...
            <h2 className="text-lg font-semibold text-foreground mb-1">Connecting to Google Drive</h2>
